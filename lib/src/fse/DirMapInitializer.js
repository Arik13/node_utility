...
const Directory_1 = require("ts_utility/Directory");
const fs = require("fs");
...
let dirMapFromPath = (rootPath, options = {}) => {
    ...
    (0, exports.traverseDirPaths)(rootPath, handler((pd, parent) => dirMap.createDir(pd.name, parent.id)), handler((pd, parent) => dirMap.createAssetDir(pd.name, parent.id, pd.name, pd.ext)));
